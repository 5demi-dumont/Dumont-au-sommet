...
\chapter{Algèbre linéaire}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item Soit $V$ un $\C$-espace vectoriel de dimension $p$.\\
		Démontrer que $V$ est un $\R$-espace vectoriel de dimension $2p$.
	...
	\section{Passage du complexe au réel}
	...
	\section{Indépendance des fonctions}
	\label{Indépendance des fonctions}
	\textcolor{blue}{\hyperref[Indépendance des fonctions corrigé]{[Corrigé]}}\\ 
	
	Soient $X$ un ensemble et $(f_1,\dots,f_n)$ une famille libre de $\C^X$.\\
	 Montrer qu'il existe $x_1,\dots,x_n\in X$ tels que ma matrice $(f_i(x_j))_{1\leq i,j\leq n}$ soit inversible.
	
	\section{L'ordre a son importance}
	...
	\section{Dimension du commutant (1)}
	\label{Dimension du commutant}
	\textcolor{blue}{\hyperref[Dimension du commutant corrigé]{[Corrigé]}}\\
	Soit $u$ un endomorphisme d'un $\C$-espace vectoriel $E$ de dimension finie.\\
	On pose $\fonction{f_u}{\mathcal L(E)}{\mathcal L(E)}{v}{u\circ v-v\circ u}$ et on note $\mathcal C(u)=\{v\in \mathcal L(E),\ u\circ v=v\circ u\}$.\\
	\begin{enumerate}
		\item Montrer que $\mathcal C(u)$ est un sous-espace vectoriel de $\mathcal L(E)$.
		\item Montrer que $\dim\mathcal C(u)\geq \dim E$.
	\end{enumerate}
	...
	\end{enumerate}
	\newpage
	\section{Lemme de Schur \ccinp{2}}
	\label{Lemme de Schur}
	\textcolor{blue}{\hyperref[Lemme de Schur corrigé]{[Corrigé]}}\\
 Soit $\K$ un corps. Soit $u$ un endomorphisme de $E$ un $\K$-espace vectoriel tel que pour tout $x\in E$, $(x,u(x))$ est une famille liée.\\
...
\chapter{Réduction géométrique}
	...
	\label{Matrices de rang 1}
	\textcolor{blue}{\hyperref[Matrices de rang 1 corrigé]{[Corrigé]}}
	\subsection{Réduction des matrices de rang $1$ \ccinp{1}} 
	\begin{enumerate}[leftmargin=*]
		\item Soit $M\in\M_{n}(\K)$. 
		Montrer que :\[\rg(M)=1 \Longleftrightarrow \exists (U,V)\in(\M_{n,1}(\K))^2 \text{ non nulles tel que } M=UV^\top\]
		\item Soit $M\in\mathcal{M}_n(\K)$ une matrice de rang $1$, montrer que $M^2=\Tr(M)M$.
	...
	Soient $A$ et $B$ deux matrices de $\M_n(\R)$ semblables sur $\C$.\\ Montrer que $A$ et $B$ sont semblables sur $\R$.
	...
	Soit $(E,\langle\ ,\ \rangle)$ un espace euclidien. On considère des vecteurs unitaires $a$ et $b$ formant une famille libre de $E$.\\
	...
	\section{Hyperplans stables \centraleponts{3}}
	\label{Hyperplans stables}
	\textcolor{blue}{\hyperref[Hyperplans stables corrigé]{[Corrigé]}}\\
	Soient $E$ un $\R$-espace vectoriel de dimension $n$ et $f\in \mathcal{L}(E)$.\\ Montrer que $f$ est diagonalisable si et seulement s'il existe des hyperplans $H_1,\dots,H_n$ de $E$ stables par $f$ tels que $\displaystyle\bigcap\limits_{i=1}^n{H_i}=\{0_E\}$.
	
	\section{Endomorphisme qui stabilise un nombre fini de sous-espaces \centraleponts{3}}
	\label{Endomorphisme qui stabilise un nombre fini de sous-espaces}
	\textcolor{blue}{\hyperref[Endomorphisme qui stabilise un nombre fini de sous-espaces corrigé]{[Corrigé]}}\\
	Soit $u$ un endomorphisme à spectre simple d'un $\K$-espace vectoriel $E$ de dimension $n$ finie. (c'est à dire que $u$ a exactement $n$ valeurs propres distinctes)\\
	...
	Soit $E$ un $\K$-espace vectoriel de dimension $n$ finie.\\ On considère $u\in \mathcal{L}(E)$ nilpotent d'indice $n$.\\
	Montrer que $u$ stabilise exactement $n+1$ sous-espaces vectoriels de $E$.
	
	\section{Sous-espaces stables par les endomorphismes de permutation \xens{4}}
	\label{Sous-espaces stables par les endomorphismes de permutation}
	\textcolor{blue}{\hyperref[Sous-espaces stables par tous les endomorphismes de permutation corrigé]{[Corrigé]}}\\
	Soit $\sigma\in \mathcal S_n$. On définit l'endomorphisme $u_\sigma : (x_1,\dots,x_n)\in \C^n\mapsto (x_{\sigma(1)},\dots,x_{\sigma(n)})$.
	\begin{enumerate}
...
\end{enumerate}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item Soit $A\in \Or_n(\R)$ telle que $1$ et $-1$ ne soient pas valeurs propres de $A$. Montrer que le polynôme minimal de $A$ est de degré pair.
	\end{enumerate}
...
\chapter{Groupes}
	...
	\section{\underline{Groupe d'exposant inférieur à 2} \ccinp{1}}
	\label{Groupe d'exposant inférieur à 2}
	\textcolor{blue}{\hyperref[Groupe d'exposant inférieur à 2 corrigé]{[Corrigé]}}\\
	...
	\section{Théorème de Dixon \xens{4}}
	...
	\label{Une relation utile sur les morphismes dee groupes}
	\textcolor{blue}{\hyperref[Une relation utile sur les morphismes de groupes corrigé]{[Corrigé]}}\\
	Soit $G$ un groupe fini et $G'$ un groupe. Soit $f:G\to G'$ un morphisme de groupe.\\
	...
	\section{Morphismes de $\Z/n\Z$ dans $\Z/m\Z$}
	...
	\section{Morphisme de $\Z^\N$ presque nul \centraleponts{3}}
	\label{Morphisme de Z^N presque nul}
	\textcolor{blue}{\hyperref[Morphisme de Z^N presque nul corrigé]{[Corrigé]}}\\
	...
	\section{\underline{Théorème de Lagrange} \centraleponts{3} (HP)}
	...
	\section{Groupe d'ordre premier \telecom{1}}
	...
	\section{Un groupe d'inversible non cyclique}
	\label{Un groupe d'inversible non cyclique}
	\textcolor{blue}{\hyperref[Un groupe d'inversible non cyclique corrigé]{[Corrigé]}}\\
	...
	\section{Ordre dans un groupe de cardinal pair}
	...
	\section{Passage par les groupes}
	...
	\section{Sous-groupe engendré par les nombres premiers}
	...
	\section{Sous-groupe engendré par le complémentaire d'un sous-groupe}
	...
	\section{Partie génératrice de $\Or(E)$ \xens{4}}
	\label{Partie génératrice de O(E)}
	\textcolor{blue}{\hyperref[Partie génératrice de O(E) corrigé]{[Corrigé]}}\\
	Soit $E$ un espace euclidien. Montrer que $\Or(E)$ est engendré par les réflexions.
	...
	\begin{enumerate}[leftmargin=*]
		\item Déterminer les sous-groupes finis de SO$(E)$.
		\item Caractériser les sous-groupes finis de $\Or(E)$.
		\item Soit $G$ un sous-groupe fini de GL$(E)$. Déterminer un produit scalaire pour lequel tous les éléments de $G$ sont des endomorphismes orthogonaux.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}[label=\alph*.]
			...
			\item Montrer que $\Or_2(\Q)=\M_2(\Q)\cap\Or_2(\R)$ est un groupe puis que SO$_2(\Q)=\{M\in \Or_2(\Q),\ \det(M)=1\}$ est un sous-groupe commutatif de $\Or_2(\Q)$. Démontrer que le sous-groupe dérivé de $\Or_2(\Q)$ est un sous-groupe strict de SO$_2(\Q)$.
		\end{enumerate}
	\end{enumerate}
...
\chapter{Anneaux et corps}
	...
	\begin{enumerate}
		...
		\textit{Indication : on pourra voir $Q_k$ comme un certain polynôme caractéristique}
...
\section{\underline{Théorème d'interversion de Möbius} \centraleponts{3}}
\label{Théorème d'intersersion de Möbius}
\textcolor{blue}{\hyperref[Théorème d'interversion de Möbius corrigé]{[Corrigé]}}\\
...
\begin{enumerate} 
	...
	\begin{enumerate}[label=\alph*.]
		...
		\item Etablir que : $$\sum_{k=1}^n \ln k=n\ln n-n+\bigO{\ln n}$$
...
Pour $\alpha\in\R$, on pose $$\forall n\in\N, \binom{\alpha}{n}=\frac{\alpha(\alpha-1)\dots(\alpha-n+1)}{n!}$$
On a également $$\forall n\in \N, \forall (a,b)\in\R ,\sum_{k=0}^n\binom{a}{k}\binom{b}{n-k}=\binom{a+b}{n}$$
...
Quel est le cardinal de $\Or_n(\R) \cap \M_n(\Z)$ ?

\section{Nombre de carrés inférieur à un entier fixé \etoile{1}}
\label{Nombre de carrés inférieur à un entier fixé}
\textcolor{blue}{\hyperref[Nombre de carrés inférieur à un entier fixé corrigé]{[Corrigé]}}\\
Pour tout $n\in \N$, on pose $A(n)$ l'ensemble des carrées d'entiers non nuls inférieur à $n$. 
\\Déterminer le cardinal de $A(n)$.
...
\begin{enumerate}
	...
	\item Une partie dénombrable de $\R$ est-elle forcément dénombrable ?
\end{enumerate}
...
\subsection{Ensemble parfait de $\R$}
\begin{enumerate}[leftmargin=*]
	\item Donner un exemple d'ensemble parfait de $\R$.
	\item Montrer qu'un ensemble parfait non vide de $\R$ n'est pas dénombrable.
	\item Montrer que l'intervalle $]0,1[$ n'est pas réunion dénombrable de segments disjoints.
\end{enumerate}
...
\textcolor{blue}{\hyperref[Caractérisations des projecteurs orthogonales corrigé]{[Corrigé]}}\\
...
\begin{enumerate}
	\item Montrer que l'application $\fonction{\langle\ ;\ \rangle}{\R_{n-1}[X]^2}{\R}{(P,Q)}{\displaystyle\int_0^1P(t)Q(t)}$ définie un produit scalaire sur $\R_{n-1}[X]$.
...
\begin{enumerate}[leftmargin=*]
	\item Soit $f$ un endomorphisme auto-adjoint positif d'un espace euclidien $E$. Montrer qu'il existe $g\in\mathcal{S}^+(E)$ tel que $g^2=f$.
	\item Soit $f$ un endomorphisme auto-adjoint défini positif d'un espace euclidien $E$. Montrer qu'il existe un unique $g\in\mathcal{S}^{++}(E)$ tel que $g^2=f$.
\end{enumerate}
...
Soit $A\in \Or_n(\R)$. Montrer que\\
$\left|\displaystyle\sum\limits_{1\leq i,j\leq n}m_{i,j}\right|\leq n \leq\displaystyle\sum\limits_{1\leq i,j\leq n}|m_{i,j}| \leq n^{3/2}$.

\section{Inégalité de la trace}
\label{Inégalité de la trace}
\textcolor{blue}{\hyperref[Inégalité de la trace corrigé]{[Corrigé]}}\\
Soient $A,B\in\mathcal S_n^+(\R)$. Montrer que $0\leq\Tr(AB)\leq\Tr(A)\Tr(B)$.
...
Soit $S=(s_{i,j})\in\mathcal S_n^+(\R)$.
...
\begin{enumerate}
	...
	\item Soient $\alpha=(\alpha_1,\dots,\alpha_n)\in\R^n$, $D=\text{diag}(\alpha_1,\dots,\alpha_n)$ et $S_{\alpha}=DSD$.\\Vérifier $S_{\alpha}\in\mathcal{S}_n^+(\R)$ et exprimer $\Tr(S_{\alpha})$.
	\item On suppose $S$ inversible. Vérifier que les coefficients diagonaux de $S$ sont strictement positifs et, en introduisant des réels $\alpha_1,\dots,\alpha_n$ pertinemment choisis, établir que: $$\det(S)\leq \displaystyle\prod\limits_{i=1}^n s_{i,i}$$
...
Soit $A\in \M_n(\R)$ la matrice Atilla $A=\begin{pmatrix}
	...
	1&\cdots&\cdots&1
\end{pmatrix}$.\\
Quelles sont les matrices $M\in \Or_n(\R)$ telles que $A+M$ soit inversible ?

\section{Somme de Cesàro de matrices orthogonales}
\label{Somme de Cesàrp de matrices orthogonales}
\textcolor{blue}{\hyperref[Somme de Cesaro de mattrices orthogonales corrigé]{[Corrigé]}}\\
Soit $A\in \mathcal{O}_n(\R)$ telle que $-1\notin \text{Sp}(A)$.
\begin{enumerate}
...
\begin{enumerate}
	...
	\item Démontrer que l'application $\fonction{\varphi}{\A_n(\R)}{\{\Omega\in \Or_n(\R)|-1\notin \text{Sp}(\Omega)\}}{A}{(A+I_n)(A-I_n)^{-1}}$ est bijective.
	\item En déduire que si $\Omega$ est une matrice orthogonale et que $-1\notin \text{Sp}(\Omega)$ alors $\det(\Omega)=1$. Pouvait-t-on constater cela d'une autre manière ?
\end{enumerate}
...
\begin{itemize}
	\item Il existe $B\in\Or_n(\R)$ telle que $A=B+B^\top$,
...
Montrer que $\exp:\A_n(\R)\to \Or_n(\R)$ est une application surjective. 
\\
\textit{\underline{Remarque:} La décomposition polaire sur $\C$ est donnée par l'application $\fonction{\varphi}{\R^*_+\times\R}{\C^*}{(r,\theta)}{re^{i\theta}}$. $\exp:\A_n(\R)\to \Or_n(\R)$ permet de comprendre d'où vient le nom de décomposition polaire pour les matrice inversible.}
...
\section{\underline{Réduction des matrices antisymétriques} \centraleponts{4}}
\label{Réduction des matrices antisymétriques}
\textcolor{blue}{\hyperref[Réduction des matrices antisymétriques corrigé]{[Corrigé]}}\\
Soit $A\in\M_n(\R)$ une matrice antisymétrique.
\\Montrer que, par le biais d'une matrice de passage orthogonale, la matrice $A$ est semblable à une matrice diagonale par blocs avec sur la diagonale des zéros et/ou différents blocs $M_{\alpha}=\begin{pmatrix}
	0&-\alpha\\ \alpha &0
\end{pmatrix}\quad \text{avec}\; \alpha\in\R^*_+$.
...
On munit $\mathcal{M}_n(\R)$ muni de son produit scalaire canonique. On note $\mathcal{A}_n(\R)$ l'ensemble des matrices antisymétriques et $\mathcal{S}_n^+(\R)$ l'ensemble des matrices symétriques positives.
\\ Soit $A\in\M_n(\R)$: $$\forall U\in\mathcal{O}_n(\R), \quad \Tr(AU)\leq\Tr(A)$$
\begin{enumerate}
	\item Déterminer le supplémentaire orthogonal de $\mathcal{A}_n(\R)$.
	\item Soit $B\in\mathcal{A}_n(\R)$. Montrer que $\forall x\in\R,\quad\exp(xB)\in\Or_n(\R)$.
	\item Montrer que $A\in\mathcal S_n^+(\R)$.
...
\begin{enumerate}
	\item Montrer que, pour tout $M\in\M_n(\R)$ et pour tout $P$ et $Q$ dans $\Or_n(\R)$, on a: $\norme{PMQ}_F=\norme{M}_F$.
...
\begin{enumerate}[resume]
	...
	\item Soient $n\geq 2$ et $M=(m_{i,j})\in\B_n(\R)$ une matrice différente de l'identité. On note $i$ le plus petit entier appartenant à $\crblanc{1}{n}$ tel que $m_{i,i}\ne1$. Montrer qu'il existe une matrice $M'=(m_{i,j}')\in\B_n(\R)$ telle que $f(M')\leq f(M)$ et $m_{j,j}=1$ pour tout $j\in\crblanc{1}{n}$.
...
Soient $E$ un espace vectoriel euclidien de dimension $n$ et $f\in \Or_n(E)$.
...
\begin{enumerate}[leftmargin=*]
	...
	\begin{enumerate}[label=\alph*.]
		...
		\item En déduire qu'il existe un unique couple $(O,S)\in \Or_n(\R)\times S^{++}_n(\R)$ tel que $A=OS$.
	\end{enumerate}
	\item Soit $A\in \M_n(\R)$.\\
	Montrer qu'il existe un couple $(O,S)\in \Or_n(\R)\times S^+_n(\R)$ tel que $A=OS$. Ce couple est-il unique ?
\end{enumerate}
...
\begin{enumerate}
	...
	\item Montrer que $\Or_n(\R)$ est un sous-groupe compact de GL$_n(\R)$.
	\item Soit $G$ un sous-groupe compact de GL$_n(\R)$ qui contient $\Or_n(\R)$. On cherche à montrer que $G=\Or_n(\R)$. Pour cela on fixe $A\in G$ et on note $A=OS$ sa décomposition polaire.
	\begin{enumerate}[label=\alph*.]
...
\begin{enumerate}
	...
	\item Montrer que l'enveloppe convexe (cf. VIII-36 tome Analyse) de $\Or_n(\R)$ est contenue dans $\B$.
	\item On suppose qu'il existe une matrice $M\in \B$ qui n'est pas dans l'enveloppe convexe de $\Or_n(\R)$. On admet que Conv$(\Or_n(\R))$ est une partie compacte de $\M_n(\R)$ (cf. VIII-37 tome Analyse) et on note $N$ le projeté convexe de $M$ sur Conv($\Or_n(\R))$ pour le produit scalaire $\langle\ ,\ \rangle:(A,B)\in \M_n(\R)^2\mapsto \Tr(A^\top B)$ et on pose $A=(M-N)^\top$. On écrit enfin $A=US$ une représentation polaire de $A$.
	\begin{enumerate}[label=\alph*.]
		\item Montrer que $\forall V\in \text{Conv}(\Or_n(\R)),\ \Tr(AV)\leq\Tr(AN)<\Tr(AM)$.
...
\begin{enumerate}[leftmargin=*]
	\item Montrer que $\Or_n(\R)$ et $T_n^{++}(\R)$ (ensemble des matrices triangulaires supérieures donc tous les coefficients diagonaux sont strictement positifs) sont des sous-groupes de $(\text{GL}_n(\R),\times)$.
	\item Montrer que $\Or_n(\R)\cap T_n^{++}(\R)=\{I_n\}$.
	...
	\\ A l'aide de ces bases, montrer qu'il existe un unique couple de matrices $(Q,R)\in \Or_n(\R)\times T_n^{++}(\R)$ tel que $A=QR$
\end{enumerate}
...
\subsection{Passage du complexe au réel}	
...
\textcolor{blue}{\hyperref[Dimension de R en tant que Q espace vectoriel ]{[Enoncé]}}\\
	...
	\subsection{Indépendance des fonctions}
	\label{Indépendance des fonctions corrigé}
	\textcolor{blue}{\hyperref[Indépendance des fonctions]{[Enoncé]}}\\
	
	
	\subsection{L'ordre a son importance}
	...
	\subsection{Centre de $\M_n(\K)$ \etoile{2}}
	...
	\subsection{Racine carrée de la dérivation \etoile{3}}
	...
	\subsection{Produit de matrices nilpotentes \etoile{5}}
	...
	\subsection{Dimension du commutant (1)}
	\label{Dimension du commutant corrigé}
	\textcolor{blue}{\hyperref[Dimension du commutant]{[Enoncé]}}\\
	\begin{enumerate}[leftmargin=*]
		\item Soit $v,w\in \mathcal{L}(E)$ et $\lambda\in \C$.
	...
	\label{Lemme de Schur corrigé}
	\textcolor{blue}{\hyperref[Lemme de Schur]{[Enoncé]}}\\
	Soient $x_1 , x_2 \in E$. D'après l'hypothèse de l'énoncé, il existe $\lambda_1 , \lambda_2 \in \K$ tel que $u(x_1)=\lambda_1x_1$ et $u(x_2)=\lambda_2x_2$.
	\begin{itemize}
	...
	\subsection{Transmission d'information \etoile{3}}
	...
	\textcolor{blue}{\hyperref[Condition nécessaire et suffisante pour que l'image et le noyau soient supplémentaires]{[Enoncé]}}\\
		...
		Montrons que (i)$\Longrightarrow$(ii)\\
		Soit $y\in\Ima(f)$\\
		...
		Montrons que (ii)$\Longrightarrow$(iii).\\
		...
		Montrons que (iii)$\Longrightarrow$(i)\\
...
\section{Correction Réduction géométrique}
	\subsection{Matrices de rang $1$}
	\label{rg1}
	\label{Matrices de rang 1 corrigé}
	\textcolor{blue}{\hyperref[Matrices de rang 1]{[Enoncé]}}
	\subsubsection{Réduction des matrices de rang $1$}
	\begin{enumerate}
		
		\item Soit $M\in\mathcal{M}_n(\K)$. \\ $\begin{array}{ccc}
			& \operatorname{rg}(M)=1\\
			\mbox{ssi} & \exists (x_1,\dots,x_n)\in\R^n, \exists U\in\mathcal{M}_{n,1}(\K)\text{ tel que }M=\left(x_1U|\dots|x_nU\right) \\
			\mbox{ssi} & \exists (U,V)\in\mathcal{M}_{n,1}(\K)\text{ tel que }M=UV^\top
		\end{array}$
		\item Puisque $m$ est de rang 1, on en déduit qu'il existe $(U,V)\in \M_{n,1}(\K)^2$ tel que $M=UV^\top$. \\On remarque que $\Tr(M)=V^\top U$. Ainsi, on a aisément $M^2=\Tr(M)M$
		\item Soit $M\in\mathcal{M}_n(\K)$ tq $\operatorname{rg}(M)=1$ \\ Donc d'après le théorème du rang, on a: $\operatorname{dim}(\operatorname{Ker}(M))=n-1$. \\On en déduit que la multiplicité algébrique de $0$ est au moins $n-1$ càd $m_0(M)\geq n-1$.
		...
		\begin{itemize}
			...
			\item Si $\Tr(M)\ne 0$, $m_0(M)=n-1$ et $\operatorname{dim}(E_{\Tr(M)}(M)=1=m_{\Tr(M)}(M)$. Donc $M$ est diagonalisable.
		\end{itemize}
		On conclut donc que $M$ est diagonalisable ssi $\Tr(M)\ne0$.
		\item Soit $A\in\mathcal{M}_3(\C)$ tel que $A^3=\begin{pmatrix}
			1 &-2 &1\\
			1 &-2 &1\\
			1 &-2 &1\\
		\end{pmatrix}=M$
		\\$M$ est de rang $1$ ainsi on a: $M^2=\Tr(M)M=0_3$ car $\Tr(M)=0$.
		\\Donc $A^6=0$.
	...
	\subsubsection{Diagonalisabilité d'une matrice}
	...
	On choisit $\lambda$ tel que $\diag(\lambda-a_1+b_1,\dots,\lambda-a_n+b_n)$ est inversible. Donc: \begin{align*}
		...
		\\&=\displaystyle\prod\limits_{i=1}^n(\lambda-a_i+b_i)(1-\displaystyle\sum_{i=1}^n\frac{b_i}{\lambda-a_i+b_i})
		\\&=\displaystyle\prod\limits_{i=1}^n(\lambda-a_i+b_i)-\sum\limits_{i=1}^n b_iP_i(\lambda)
	\end{align*}
	Avec $P_i(\lambda)=\displaystyle\prod\limits_{j\in\crblanc{1}{n}, j\ne i}(\lambda-a_i+b_i)$.
	\\Puisque $a_1\leq\dots\leq a_n$ et $0<b_n<\dots< b_1$, on a $a_1-b_1<\dots<a_n-b_n$.
	\\On a $\chi_M(a_i-b_i)=-b_i\displaystyle\prod\limits_{j\in\crblanc{1}{n}, j\ne i}[(a_i-b_i)-(a_j-b_j)]$.
	...
	\subsection{Matrices réelles semblables}
	...
	\begin{enumerate}
		...
		\item Puisqu'une matrice est inversible si et seulement si son déterminant est non nul, d'après la question précédente, $A \times B$ est inversible si et seulement si $A$ et $B$ le sont. Dans ce cas, on a d'après la première question
		$$(A \otimes B) \cdot (A^{-1} \otimes B^{-1}) = (AA^{-1}) \otimes (BB^{-1}) = I_2 \otimes I_2 = I_4$$
		\item D'après l'énoncé, il existe $P , Q \in Gl_n(\K)$ et $D_A , D_B \in \M_{n}(\K)$ diagonales tels que : $A=PD_AP^{-1}$ et $B=QD_BQ^{-1}$.\\
		On montre aisément que : $$(A\otimes B)=(P\otimes Q)(D_A\otimes D_B)(P^{-1}\otimes Q^{-1})$$
	...
	\begin{enumerate}[leftmargin=*]
			...
			On choisit un $i$ pour lequel $|x_i|$ est maximal. Ainsi, 
			\begin{align*}
				|\lambda-a_{i,i}|&=|\sum_{j\ne i}a_{i,j}\frac{x_j}{x_i}|\\
				&\leq\sum_{j\ne i}|a_{i,j}|\frac{|x_j|}{|x_i|}\\
				&\leq\sum_{j\ne i}|a_{i,j}|=R_i &\mbox{car $|x_i|$ est maximal}
			\end{align*}
			Par conséquent, on a : \[\lambda \in D(a_{i,i},R_i)\]
			\item Montrons que $0\not\in D(a_{i,i},R_i)$ pour tout $i\in\crblanc{1}{n}$.
			Soit $i\in\crblanc{1}{n}$.\\
			\[|0-a_{i,i}|=|a_{i,i}|>R_i\]
			donc $0\not\in D(a_{i,i},R_i)$.\\
	...
	\subsection{Spectre de $u\circ v$ et $v\circ u$}
	...
	\end{align*}
	\subsection{Endomorphismes qui commutent}
	...
	$v_{E_\lambda(u)}$ admet un vecteur propre car $E$ est un $\C$-espace vectoriel.\\
	Ce vecteur propre est un vecteur propre de $v$ et un vecteur propre de $u$, donc $u$ et $v$ admettent un vecteur propre commun.
	
	\subsection{Vecteur propre commun}
	...
	\subsection{Endomorphisme qui stabilise un nombre fini de sous-espaces \etoile{3}}
	\label{Endomorphisme qui stabilise un nombre fini de sous-espaces corrigé}
	\textcolor{blue}{\hyperref[Endomorphisme qui stabilise un nombre fini de sous-espaces]{[Enoncé]}}\\
	...
	\subsection{Sous-espaces stables d'un endomorphisme nilpotent maximal \etoile{3}}
	...
	\subsection{Sous-espaces stables par tous les endomorphismes de permutation \etoile{4}}
	\label{Sous-espaces stables par tous les endomorphismes de permutation corrigé}
	\textcolor{blue}{\hyperref[Sous-espaces stables par les endomorphismes de permutation]{[Enoncé]}}\\
	\begin{enumerate}[leftmargin=*]
	...
	\subsection{Endomorphismes diagonalisables d'un $\R$-espace vectoriel}
...
\section{Correction Réduction algébrique}
	...
	Pour $\lambda\in \C$ on note $m_\lambda$ la multiplicité de $\lambda$ en tant que valeur propre de $A$, càd en tant que racine de son polynôme caractéristique ($m_\lambda=0$ si $\lambda$ n'est pas valeur propre de $A$).\\
...
Pour le cas $n=2$, on écrit $A=\begin{pmatrix}a&b\\c&d\end{pmatrix}$. On calcule $\Com(A)=\begin{pmatrix}d&-c\\-b&a\end{pmatrix}$ puis $A+\tilde A=\begin{pmatrix}a+d&0\\
...
Reste à traiter le cas $\Delta=0$ càd $\lambda^2=4\det(A)$. On note $\mu=\dfrac{\lambda}{2}\ne 0$ la racine double de $P$. On a $\det(A)=\mu^n=\mu_2$ donc $\mu$ est une racine $(n-2)$-ième de l'unité.\\
...
Finalement $\T{Sp}(\tilde A)=\{\det(A)\mu^{-1},\ \mu\in \T{Sp}(A)\}$.
	...
	En outre, d'après le théorème de Cayley-Hamilton $(S-\Id)^n=0$.
...
\section{Correction Groupes}
	\subsection{Existence d'un idempotent \etoile{3}}
	...
	\subsection{Sous-semi-groupes finis de $\M_n(\K)$}
	...
	\subsection{Groupe d'exposant inférieur à $2$ \etoile{1}}
	\label{Groupe d'exposant inférieur à 2 corrigé}
	\textcolor{blue}{\hyperref[Groupe d'exposant inférieur à 2]{[Enoncé]}}\\
	...
	\subsection{Centre et Commutant \etoile{1}}
	...
	\subsection{Théorème de Dixon \etoile{4}}
	...
	\subsection{Opérations sur les sous-groupes \etoile{2}}
	...
	\subsection{Sous-groupes finis de $\U$ \etoile{2}}
	...
	\subsection{Groupes quasi-cycliques de Prüfer \etoile{3}}
	...
	\subsection{Sous-groupes de GL$_n(\C)$ qui intersectent trivialement le groupe spécial linéaire \etoile{3}}
	...
	\subsection{Matrices inversibles à coefficients entiers}
	...
	\subsection{Sous-groupes de $(\Z,+)$ \etoile{2}}
	...
	\subsection{Sous-groupes de $(\R,+)$ \etoile{3}}
	...
	\subsubsection{Fonction (multi)périodique \etoile{3}}
	...
	\subsubsection{Valeur d'adhérence du cercle unité \etoile{3}}
	...
	\subsection{Sous-groupes distingué}
	...
	\subsection{Une relation utile sur les morphismes de groupes}
	\label{Une relation utile sur les morphismes de groupes corrigé}
	\textcolor{blue}{\hyperref[Une relation utile sur les morphismes dee groupes]{[Enoncé]}}\\
	...
	Ces classes d'équivalence sont les $g\Ker(f)=\{gk , k\in\Ker(f)\}$ pour $g\in G$.\\
	...
	Finalement, puisque les classes d'équivalences forment une partition de $G$, on en déduit : $$|G|=\sum_{g\in\Ima(f)}|g\Ker(f)|=\sum_{g\in\Ima(f)}|\Ker(g)|=|\Ker(f)||\Ima(f)|$$
	
	\subsection{Automorphisme d'inversion \etoile{1}}
	...
	\subsection{Automorphismes intérieurs \etoile{1}}
	...
	\subsection{Endomorphismes continus de $\R$ \etoile{2}}
	...
	\subsection{Morphismes de $\Q$ dans $\Z$ \etoile{1}}
	...
	\subsection{Morphismes de $\Z/n\Z$ dans $\Z/m\Z$}
	...
	\subsection{Morphismes de GL$_n(\R)$ dans $\Z /m\Z$ \etoile{5}}
	...
	\subsection{Quasi-morphisme \etoile{3}}
	...
	\begin{enumerate}[leftmargin=*]
		...
		$|f(y)|=\unfty\lim\displaystyle\frac{|f(yz_n)|}{|f(z_n)|}$. Donc $(|f(yz_n)|)_{n\in \N}$ diverge vers $+\infty$ et on peut écrire comme $f$ ne s'annule pas :
		$$f(xy)=\unfty\lim\displaystyle\frac{f(xyz_n)}{f(z_n)}=\unfty\lim\frac{f(xyz_n)}{f(yz_n)}\cdot\frac{f(yz_n)}{f(z_n)}=f(x)f(y)$$
	\end{enumerate}
	
	\subsection{Morphisme de $\Z^\N$ presque nul \etoile{3}}
	\label{Morphisme de Z^N presque nul corrigé}
	\textcolor{blue}{\hyperref[Morphisme de Z^N presque nul]{[Enoncé]}}\\
	\begin{enumerate}[leftmargin=*]
	...
	Notons $E$ le neutre de $G$.\\
	Par définition du neutre $E^2=E$. Ainsi $E$ est une matrice de projection. On sait (ou redémontre aisément) alors que $E$ est semblable, par la matrice de passage $P$ de la base canonique de $\K^n$ à une base adaptée à la décomposition $\M_n(\K)=\ker(E-I_n)\bigoplus\ker(E)$, à $J_r=\left(\begin{array}{c|c}
		I_r&0\\
		\hline
		0&0
	\end{array}\right)$.\\
	...
	\subsection{Caractérisation de la finitude d'un groupe par ses sous-groupes \etoile{3}}
	...
	\subsection{Sous-groupe des éléments d'ordre fini \etoile{4}}
	...
	\subsection{Théorème de Lagrange \etoile{3}}
	...
	\subsection{Un cas particulier du lemme de Cauchy \etoile{4}}
	...
	\subsection{Groupe d'ordre premier \etoile{1}}
	...
	\subsubsection{Plus petit groupe non commutatif}
	\begin{enumerate}
		...
		\begin{itemize}
			\item Si $G$ contient un élément d'ordre $4$. On note cet élément $a$. Le sous-groupe cyclique engendré par $a$ contient 4 éléments distincts. Ainsi, $\langle a\rangle=G$ Donc $G$ est cyclique donc $G$ est commutatif.
			\item Si tous les éléments non neutres de $G$ sont d'ordre $2$. D'après l'exercice V.3, $G$ est commutatif.\\
		\end{itemize}
		Finalement, aucun groupe d'ordre 4 n'est non commutatif.
		Cet entier est 6. Il suffit de regarder le groupe symétriques $S_3$ qui n'est pas commutatif.
		
	\end{enumerate}
	\subsection{Sous-groupe d'un groupe cyclique \etoile{3}}
	...
	Ainsi $\exists q\in \Z,\ k=q\cdot\displaystyle\frac{n}{d}$. On a alors $z=x^{q\cdot n/d}=y^q$ : $z\in <y>$.\\
	Finalement $H\subset <y>$ et comme $|H|=d=|<y>|,\ H=<y>$.
	
	\subsection{Exposant d'un groupe abélien fini \etoile{4}}
	...
	\subsection{Un groupe d'inversible non cyclique}
	\label{Un groupe d'inversible non cyclique corrigé}
	\textcolor{blue}{\hyperref[Un groupe d'inversible non cyclique]{[Enoncé]}}\\
	
	\subsection{Ordre dans un groupe de cardinal pair}
	...
	\subsection{Ordre dans $\Z/n\Z$ \etoile{2}}
	\label{Ordre dans Z/nZ corrigé}
	\textcolor{blue}{\hyperref[Ordre dans Z/nZ]{[Enoncé]}}\\
	D'une part, $\displaystyle\frac{k}{k\land n}\in \N\implies\frac{n}{n\land k}\overline{k}=\frac{k}{n\land k}\overline{n}=\overline{0}$. Donc $d|\displaystyle\frac{n}{n\land k}$.\\\\
	D'autre part, $\overline{dk}=d\overline{k}=\overline{0}$ donc $n|dk$.\\
	Alors $\displaystyle\frac{n}{n\land k}|d\cdot\frac{k}{n\land k}$. Or $\pgcd\left(\displaystyle\frac{n}{n\land k},\frac{k}{n\land k}\right)=1$ donc d'après le lemme de Gauss, $\displaystyle\frac{n}{n\land k}|d$.\\
	Ainsi comme $d\geq 0$ et $\displaystyle\frac{n}{n\land k}\geq 0,\ d=\frac{n}{n\land k}$.
	
	\subsection{Passage par les groupes}
	...
	\subsection{Groupe infini non monogène \etoile{1}}
	\label{Groupe infini non monogène corrigé}
	\textcolor{blue}{\hyperref[Groupe infini non monogène]{[Enoncé]}}\\
	$(\Z^2,+)$ n'est pas monogène.\\
	$\forall (a,b)\in \Z^2,\ <(a,b)>=\{(ka,kb),\ k\in \Z\}\ne \Z^2$.
	
	\subsection{Groupe non cyclique \etoile{2}}
	...
	\subsection{Ordre dans le groupe symétrique \etoile{1}}
	...
	\subsection{Sous-groupe engendré par les nombres premiers}
	...
	\subsection{Sous-groupe engendré par le complémentaire d'un sous-groupe}
	...
	\subsection{Groupe alterné $\etoile{3}$}
	...
	\subsection{Cardinal minimal d'une famille de transpositions engendrant $\mathcal{S}_n$}
	...
	\begin{enumerate}[leftmargin=*]
		\item On rappelle que $\mathcal{S}_n$ est engendré par les transpositions.\\
	...
	\begin{enumerate}[leftmargin=*]
		...
		On a montré que $\mathcal P_n\subset \Or_n(\R)$.
	...
	\subsection{Groupe dérivé \etoile{5}}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\begin{enumerate}[label=\alph*.]
			...
			\item SO$_2(\Q)\subset \text{SO}_2(\R)$ qui est un groupe abélien donc les éléments de SO$_2(\Q)$ commutent tous entre eux. $I_2\in\Or_2(\Q)$. De plus, $\det(\Or_2(\Q))\subset \det(\Or_2(\R))=\{-1,1\}$. Donc si $M=\begin{pmatrix}a&b\\c&d\end{pmatrix}$ et $M'=\begin{pmatrix}a'&b'\\c'&d'\end{pmatrix}$ sont dans $\Or_2(\Q)$ alors en notant $\varepsilon=\displaystyle\frac{1}{\det(M(M')^{-1})}\in \{-1,1\}$,\\    $M(M')^{-1}=\begin{pmatrix}a&b\\c&d\end{pmatrix}\cdot\varepsilon\begin{pmatrix}d'&-b'\\-c'&d'\end{pmatrix}=\varepsilon\begin{pmatrix}ad'-bc'&bd'-ab'\\cd'-dc'&dd'-db'\end{pmatrix}\in \M_2(\Q)$ d'où $M(M')^{-1}\in \Or_2(\Q)$. De plus, si $\det(M)=\det(M')=1$ alors $\det(M(M')^{-1})=1$ donc SO$_2(\Q)$ est un sous groupe de $\Or_2(\Q)$.\\
			Il est évident que $D\subset \text{SO}_2(\Q)$. Supposons par l'absurde que $D=C=\text{SO}_2(\Q)$. Comme tous les éléments de $\Or_2(\Q)\backslash \text{SO}_2(\Q)$ ont un carré égal au neutre, $C$ est engendré par les carrés des éléments de $\text{SO}_2(\Q)$.\\
			Ensuite, les éléments de $\Or_2(\Q)$ de déterminant $-1$ sont des réflexions.\\
			...
			\end{pmatrix}\in \Or_2(\Q),\ B=\begin{pmatrix}1&0\\0&-1
			\end{pmatrix}\in \Or_2(\Q),\ A^2=B^2=I_2$.\\
			Ainsi $\Or_2(\Q)$ est bien engendré par les involutions d'où $D=C$ d'après la question $2$.\\
			Or comme SO$_2(\Q)$ est commutatif, un produit de carré de matrices de SO$_2(\Q)$ est encore un carré dans SO$_2(\Q)$.\\
			Et donc pour montrer que $D$ est distinct de SO$_2(\Q)$ il suffit de montrer qu'il existe une matrice de SO$_2(\Q)$ qui n'est pas un carré dans SO$_2(\Q)$. C'est le cas de $M=\begin{pmatrix}0&-1\\1&0\end{pmatrix}$. Supposons qu'il existe une matrice $N\in \Or_2(\Q)$ telle que $M=N^2$. On a forcément $N\in \text{SO}_2(\Q)$ puisque sinon $N^2=I_n$. On peut donc écrire $N=\begin{pmatrix}\cos(\theta)&-\sin(\theta)\\\sin(\theta)&\cos(\theta)\end{pmatrix}$ avec $\theta\in \R$. Mais alors $2\theta\equiv \displaystyle\frac{\pi}{2}[2\pi]$ et donc $\cos(\theta)=\pm\displaystyle\frac{\sqrt{2}}{2}\notin \Q$ ce qui est absurde.
		\end{enumerate}
	\end{enumerate}
	
	\subsection{Sous-groupe discret de $\C$ et de SL$_2(\R)$ \etoile{5}}
...
\section{Correction Anneaux et corps}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item \begin{enumerate}[label=\alph*.]
			...
			\item On montre que $Z(A)$ est un sous-anneau (cf. \ref{Centre d'un anneau}). Fixons $x\in A$.\\
	...
	L'application $\fonction{\Phi}{\Q_1[X]}{\Q[\sqrt2]}{P}{P(\sqrt 2)}$ est un morphisme d'algèbre.\\
...
\section{Correction Arithmétique}
	...
	\begin{enumerate}[leftmargin=*]
		...
		Ainsi puisque $\mathcal{C}$ est l'image de cette application et que $\Card(\F_p^*)=p-1$, on obtient : \[\Card(\mathcal{C})=\frac{p-1}{2}\]
		\item Puisque $\Card(\F_p^*)=p-1$, on a $\left(a^{\frac{p-1}{2}}\right)^2=1$.\\
		Ainsi par intégrité du corps $\F_p^*$, $a^{\frac{p-1}{2}}\in\{\pm\overline{1}\}$.
		...
		Donc \[\mathcal{C}\subset\{a\in\F_p^*,a^{\frac{p-1}{2}}=\overline{1}\}\]
		On pose $P=X^{\frac{p-1}{2}}-\overline{1}\in \R_{\frac{p-1}{2}}[X]$.\\
		Supposons qu'il existe $a\in\F_p^*\setminus\mathcal{C}$ tel que $a^{\frac{p-1}{2}}=\overline{1}$.\\
		...
		Ainsi, $\mathcal{C}=\{a\in\F_p^*,a^{\frac{p-1}{2}}=\overline{1}\}$.
	\end{enumerate}
	\subsection{Indicatrice d'Euler}
	\label{Indicatrice d'Euler corrigé}
		\textcolor{blue}{\hyperref[Indicatrice d'Euler]{[Enoncé]}}\\
	\begin{enumerate}[leftmargin=*]
		\item Montrons que le seul sous-groupe d'ordre d de $\Z/n\Z$ est $\Z/d\Z$.\\
		Il est clair que $\Z/d\Z$ est un sous-groupe de $\Z/n\Z$ car $d\mid n$.\\
		Soit $H$ un sous-groupe d'ordre $d$ de $\Z/n\Z$.\\
		Pour tout $x\in H, x^d=e$ d'après le théorème de Lagrange.\\
		Donc $H\subset \Z/d\Z$.\\
		Ainsi par égalité des cardinaux, $H=\Z/d\Z$.
		Maintenant, il suffit d'énumérer les éléments d'ordre exactement $d$ qui les générateurs de $\Z/d\Z$.\\
		On en déduit donc qu'il y a $\varphi(d)$ éléments d'ordre $d$ dans $\Z/n\Z$.
		...
		\item On remarque ainsi que \[\varphi(n)=n-\sum_{d\mid n et d\ne n} \varphi(d)\]
		\begin{lstlisting}
			def ind_euler(n):
			 L=[]
			 s=0
	...
	\begin{enumerate}
		...
		L'étude de la fonction: $x\mapsto\frac{x-1}{x+k-1}$ sur $[2,+\infty[$ donne : $$\prod_{i=1}^k\left(1-\frac{1}{n_i}\right)\geq\frac{1}{k+1}.$$
	...
	\end{enumerate}
	\subsection{Théorème d'interversion de Möbius}
	\label{Théorème d'interversion de Möbius corrigé}
	\textcolor{blue}{\hyperref[Théorème d'intersersion de Möbius]{[Enoncé]}}\\
	\begin{enumerate}
		...
		Par définition de $\mu$, les seuls diviseurs positifs qui de $n$ qui vont intervenir dans la somme sont ceux de la forme :\[p_I=p_{i_1}\dots p_{i_r}\]
	 	...
	 	Or pour tout $s\in\crblanc{0}{r}$, il y a $\binom{r}{s}$ parties $I$ de $\crblanc{1}{r}$ à $s$ éléments.\\
...
\section{Correction Dénombrement}
	...
	\subsection{Nombre de carrés inférieur à un entier fixé \etoile{1}}
	\label{Nombre de carrés inférieur à un entier fixé corrigé}
		\textcolor{blue}{\hyperref[Nombre de carrés inférieur à un entier fixé]{[Enoncé]}}\\
	Fixons $n\in \N$.\\
	$\forall k\in \Z,\ k^2\in A(n)\iff 0<k^2\leq n\iff 0<|k|\leq\sqrt{n}\iff 0<|k|\leq\lfloor\sqrt{n}\rfloor$.\\
	Donc $\Card(A(n))=\lfloor\sqrt{n}\rfloor$.
	...
	\subsubsection{Ensemble parfait de $\R$}
...
\section{Correction Probabilités}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item On sait que $\V(X)=\E(|X-\E(x)|)$, donc en posant $Y=X-\E(X)$, d'après la question 1, on a que \[Y \text{ est presque surement nulle}\]
		Donc \[\p(X-\E(X)=0)=1\]
		Donc en posant $a=\E(X)$, on a le résultat.
	\end{enumerate}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item Pour $p=q=2$ on a $\E(XY)\leq \sqrt{\E(X^2)\E(Y^2)}$. Il s'agit de l'inégalité de Cauchy-Schwartz.
	\end{enumerate}
	...
	\begin{enumerate}[leftmargin=*]
		\item On note $X_i$ la variable aléatoire qui vaut $1$ si la $i$-ème ampoule est allumée. \\On a donc $X_i\sim\mathcal{B}(p_i)$.\\
		...
		Donc qu'un tel $\lambda$ existe il faut que les $p_i$ soient égaux, on a donc : \[p_i=\frac{m}{n}\]
		pour tout $i\in\crblanc{1}{n}$.
		Ainsi, la variance est maximale pour $p=\left(\frac{m}{n},\dots,\frac{m}{n}\right)$ et : \[\V(Y)=\sum_{i=1}^{n}\frac{m}{n}\left(1-\frac{m}{n}\right)=m(1-\frac{m}{n})\]
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item D'après l'inégalité de Cauchy-Schwartz, $\displaystyle\left|\operatorname{Cov}(\mathbbm{1}_A,\mathbbm{1}_B)\right|\leq\sqrt{\V(\mathbbm{1}_A)\V(\mathbbm{1}_B)}\leq\frac{1}{4}$.
	...
	\begin{enumerate}
		...
		\end{align*}
		\item D'après l'inégalité de Cauchy-Schwartz,\\
...
\section{Correction Endomorphismes d'un espace euclidien}
	...
	Soit $A\in\M_{n}(R)$ telle que $Com(A)=A$.\\
	...
	Réciproquement, il est clairement que si $A=0$ alors $\Com(A)=A$.\\
	Si $A\in\mathcal{SO}_n(\R)$ alors $A^{-1}=A^\top$ et $\det(A)=1$.\\ 
	Donc puisque \[A\Com(A)^\top=I_n\] Donc par unicité de l'inverse $\Com(A)^\top=A^\top$.
	D'où $A=\Com(A)$.\\
	Donc l'ensemble des solutions des matrices vérifiant $A=\Com(A)$ est : \[\mathcal{SO}_n(\R)\cup\{0\}\]
	...
	Donc la matrice J défini par \[J=\begin{pmatrix}
	...
	\end{pmatrix}\]
	est dans $H$.\\
	Or cette matrice est inversible donc on a montré par l'absurde que toute hyperplan de $\M_n(\K)$ rencontre $\text{GL}_n(\K)$.
	...
	\subsection{Caractérisations des projecteurs orthogonales}
	\label{Caractérisations des projecteurs orthogonales corrigé}
	...
	\begin{enumerate}
		\item \begin{enumerate}[label=\alph*.]
			...
			Puisque $(\Or(E),\circ)$ est un groupe $s_1\circ s_2\in \Or(E)$
		...
		\end{enumerate}
		\item Puisque $\Or(E)$ est un groupe, $r\circ s$ est une isométrie vectorielle.
	...
	\subsection{Réduction des matrices antisymétriques}
	\label{Réduction des matrices antisymétriques corrigé}
		\textcolor{blue}{\hyperref[Réduction des matrices antisymétriques]{[Enoncé]}}\\
...
\section{Correction Décompositions matricielles}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item \begin{enumerate}[label=\alph*.]
			...
			\item Si $A=OS$ avec $(O,S)\in \Or_n(\R)\times S_n^{++}(\R)$ alors $A^\top A=S^\top O^\top OS=S^\top S=S^2$. Donc $S$ est l'unique matrice de $S_n^{++}(\R)$ qui vérifie $A^\top A=S^2$. On a alors forcément $O=AS^{-1}$.\\
		...
		Comme $\Or_n(\R)$ est compact on peut extraire une suite $(O_{\varphi(k)})_{k\in \N}$ de $(O_k)_{k\in \N}$ qui converge. On note $O$ sa limite.\\
		...
		Finalement, $A=OS$ où $(O,S)\in \Or_n(\R)\times S_n^+(\R)$.\\
		Il n'y a pas unicité. Par exemple la matrice nulle peut s'écrire $0_n=I_n\times 0_n=-I_n\times 0_n$ où $(I_n,-I_n)\in \Or_n(\R)^2$ et $0_n\in S_n^+(\R)$.
	\end{enumerate}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\item $\Or_n(\R)$ est un sous-groupe de GL$_n(\R)$ :
		\begin{itemize}
			\item $\Or_n(\R)\subset \text{GL}_n(\R)$ ($\forall M\in \Or_n(\R),\ M^{-1}=M^\top$);
			\item $I_n\in \Or_n(\R)$;
			\item $\forall (A,B)\in \Or_n(\R)^2,\ (AB^{-1})^\top AB^{-1}=BA^\top AB^\top=BB^\top=I_n$.
		\end{itemize}
		$\Or_n(\R)$ est compact :
		\begin{itemize}
			\item $\Or_n(\R)\subset \M_n(\R)$ avec $\dim(\M_n(\R))=n^2<+\infty$;
			\item $\Or_n(\R)=f^{-1}(\{I_n\})$ pour $f:M\mapsto M^\top M$ qui est continue car les coefficients de $f(M)$ sont polynomiaux en ceux de $M$. Donc $\Or_n(\R)$ est fermé;
			\item $\forall M\in \Or_n(\R),\ {|||M|||}_2=\sup\limits_{\substack{X\in \R^n\\\normep{2}{X}=1}}\normep{2}{MX}=\sup\limits_{\substack{X\in \R^n\\\normep{2}{X}=1}}\normep{2}{X}=1$.
		\end{itemize}
		$\Or_n(\R)$ est un fermé borné de $\M_n(\R)$ qui est de dimension finie, c'est donc un compact.
		\item \begin{enumerate}[label=\alph*.]
			\item Supposons que $S$ ait une valeur propre $\lambda>1$.\\
			Comme $G$ est un groupe qui contient $\Or_n(\R)$, $\forall n\in \N^*,\ S^n=(O^{-1}A)^n\in G$.\\
			...
			\item Supposons que $S$ ait une valeur propre $\lambda<1$. Alors comme $G$ est un groupe qui contient $\Or_n(\R)$, $S^{-1}=A^{-1}O\in G$. Or $\displaystyle\frac{1}{\lambda}\in \text{Sp}(S^{-1})$ et $\displaystyle\frac{1}{\lambda}>1$. Par le même raisonnement qu'à la question précédente ceci est absurde. On en déduit que Sp$(S)=\{1\}$.
		\end{enumerate}
		\item Le théorème spectral donne que $S$ est diagonalisable. On a donc d'après la question précédente $S=I_n$.\\
		Ainsi $A=O\in \Or_n(\R)$ et finalement $G=\Or_n(\R)$.
	\end{enumerate}
	...
	\begin{enumerate}[leftmargin=*]
		...
		De plus, en notant $i_0\in \crblanc{1}{n}$ tel que $\lambda_{i_0}=\rho(A^\top A)$ on a $X^\top e_{i_0}X=\rho(A^\top A)$.\\
		Ainsi ${|||A|||}_2=\sqrt{\rho(A^\top A)}$.
		\item Soit $A\in \text{Conv}(\Or_n(\R))$. Il existe $A_1,\dots,A_p$ des matrices orthogonales ainsi que $t_1,\dots,t_p$ des réels positifs de somme $1$ tels que $A=\displaystyle\sum_{i=1}^pt_iA_i$.\\
		On remarque que $\forall M\in \Or_n(\R),\ {|||M|||}_2=\sup\limits_{\substack{X\in \M_n(\R)\\\normep{2}{X}=1}}\normep{2}{MX}=\sup\limits_{\substack{X\in \M_n(\R)\\\normep{2}{X}=1}}\normep{2}{X}=1$.\\
		Donc par inégalité triangulaire ${|||A|||}_2\leq \displaystyle\sum_{i=1}^pt_i{|||A_i|||}_2=\sum_{i=1}^pt_i=1$ d'où $A\in \B$.
		\item \begin{enumerate}[label=\alph*.]
			\item Soit $V\in \text{Conv}(\Or_n(\R))$.\\
			D'une part, $\Tr(AV)-\Tr(AN)=\Tr(A(V-N))=\proscal{M-N}{V-N}\leq 0$ d'après le théorème de projection sur un convexe compact.\\
			D'autre part, $\Tr(AM)-\Tr(AN)=\Tr((M-N)^\top(M-N))=\proscal{M-N}{M-N}\geq 0$. De plus $N\in \text{Conv}(\Or_n(\R))$ et $M\notin \text{Conv}(\Or_n(\R))$ donc $M\ne N$ d'où $\proscal{M-N}{M-N}>0$.
			\item D'après la question précédente, pour $V=U^{-1}\in \Or_n(\R)\subset \text{Conv}(\Or_n(\R)),\ \Tr(S)=\Tr(U^{-1}US)=\Tr(U^{-1}A)\leq \Tr(AN)<\Tr(AM)=\Tr(USM)$.
